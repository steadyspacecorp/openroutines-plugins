...
Email the agent's daily check-in to the team the way a teammate would:
once, at the start of the workday, covering everything since the last
one. Your input is your pending changes; your output is at most one
email to `$AGENTMAIL_REPORT_TO`. The agentmail-send skill covers
formatting and sending.

## 1. Decide whether to send
...
further, send nothing, consume nothing. A quiet inbox is the feature;
never send a "nothing to report" email.

Then your ledger, which holds the last report you delivered. If it
already covers these exact changes -- a send that landed without
consuming -- consume them now without sending again; the team already
read this report.

Send -- unless the day holds no news at all: every pending event a
...
into the next real one. News on either side sends as usual.
...
crowded inbox. Everything below follows from that.
...
- **What's next** -- required; never blank. It projects the schedule,
  ...
  transferred to Human-owned. Before sending, check the counts: one
...
Keep the whole email under a dozen short lines. The subject is the
day's headline outcome -- most recipients decide from it alone whether
to open -- never a generic label like "Daily check-in".

## 3. Send

One send, per the agentmail-send skill; delivery is HTTP 200 whose body
carries a `message_id`, and nothing else. On delivery, record the sent
report in your ledger, replacing the last one -- it's both the
double-send guard above and the file a person opens to catch up on this
agent without digging through their inbox.

## 4. Consume

A delivered email is this routine's delivery -- consume the changes. So
is a ledger match in the gate: already-delivered changes get consumed
without a second send. A failed send, a closed gate, or an all-quiet
