...
credentials:
  agentmail_api_key:
    description: An AgentMail API key scoped to the sending inbox, minted per the steps below -- message_send, inbox_read, and inbox_update for report-only, plus message_read to enable the reply loop
variables:
  agentmail_inbox_id:
...
- **Report-only**: `message_send`, `inbox_read`, and `inbox_update`. The key can send from its one inbox, confirm the wiring, and update that inbox's settings -- `inbox_update` is what lets agentmail-verify keep the sender name equal to the agent's name. It cannot read mail, touch other inboxes, or create new ones. Enough for agentmail-report and agentmail-verify.
...
2. Create the agent's inbox:
   ...
   Note the `inbox_id` in the response. Don't worry about the `display_name` (the sender name recipients see in their mail client): agentmail-verify sets it to the agent's `name` from `openroutines.yml`, on a new inbox and an existing one alike.
...
3. Mint the inbox-scoped key. For report-only:
   ...
   curl -sS -X POST https://api.agentmail.to/v0/inboxes/<inbox_id>/api-keys \
     -H "Authorization: Bearer <org-api-key>" \
     -H "Content-Type: application/json" \
     -d '{"name": "openroutines-report", "permissions": {"message_send": true, "inbox_read": true, "inbox_update": true}}'
   ```
