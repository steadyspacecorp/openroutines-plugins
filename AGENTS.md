...
- Payload is limited to `routines/`, `skills/`, `bin/` operator scripts, and `memory/ledgers/` stubs. A `bin/` script is a hand-run convenience wrapper carrying a routine invocation with its right flags: it installs executable at `plugins/<name>/bin/` and stays off PATH. Routines never see it -- the run workspace allow-list excludes it -- so the only party who can execute it is the person, by path, after the same diff review as the rest of the payload. Do not add config files, Dockerfiles, key material, nested Git metadata, symlinks, devices, or install hooks.
