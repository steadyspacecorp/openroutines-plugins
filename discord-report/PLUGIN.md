...
3. `OPENROUTINES_LOG_LEVEL=warn openroutines routines run discord-verify` -- confirms the webhook and posts one labeled test message through the real wiring.
4. Adjust the schedule, then `openroutines check`, review the diff, commit.

A manual run discards knowledge changes only; it does not suppress Discord posts or withhold credentials. `openroutines check` is the non-acting validation path.
