...
Post the agent's daily check-in to the team's Discord channel the way a
...
at most one webhook post. The discord-post skill covers formatting and
sending.
...
One webhook post with `?wait=true`, per the discord-post skill;
delivery is HTTP 200 whose body is the created message (it has an
`id`), and nothing else. On delivery, record the posted report in your
ledger, replacing the last one -- it's both the double-post guard above
and the file a person opens to catch up on this agent without Discord
scrollback.
