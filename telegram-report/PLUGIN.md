...
Telegram authenticates with the token in the URL path, and a committed trigger URL must never carry a token. So the trigger URL carries the credential's environment reference instead -- `https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/getUpdates` -- and OpenRoutines substitutes the value at poll time (0.1.0 and later). The trigger reads the queue without `offset`, so it never confirms anything, and compares the whole response, so any new update wakes the routine within a minute. The schedule (twice a workday) is only the backstop. Tighten the trigger interval or the schedule as you like; each fire that finds an empty queue ends one call in.
...
Telegram bots have no history API: `getUpdates` is a queue, held 24 hours, drained by the `offset` parameter. telegram-inbox keeps the last handled `update_id` as a watermark in its ledger and reads from there; passing the offset confirms everything older, so the routine advances the watermark only past updates it actually handled and a failed reply is simply retried next run. The queue has one consumer -- never point a webhook at the bot or run a second poller, or every read returns 409. The trigger's own read is not a second consumer: it passes no `offset` and returns in milliseconds, so it confirms nothing and does not hold the queue open. In the report group, privacy mode means the queue holds essentially only replies to the agent's reports; the trigger bounds how long an answer waits, and the 24-hour retention is why the backstop schedule should stay denser than daily.
